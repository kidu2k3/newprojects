...
@pytest.mark.asyncio
async def test_handshake_authentication(keypairs):
...
@pytest.mark.asyncio
async def test_replay_protection(keypairs):
...
@pytest.mark.asyncio
async def test_key_rotation(keypairs):
    """Test that session keys are rotated after the interval."""
...
if __name__ == '__main__':
    ...
    async def run_tests():
        ...
        try:
            await test_handshake_authentication(pairs)
            await test_replay_protection(pairs)
            await test_key_rotation(pairs)
